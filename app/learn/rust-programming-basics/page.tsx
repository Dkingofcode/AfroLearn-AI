// pages/learn/rust-programming-basics/page.tsx
"use client"
...
// Find the Rust Programming Basics module
const rustModule = courseModules.find((module) => module.slug === "rust-programming-basics")

export default function RustProgrammingBasicsPage() {
  const { t } = useLanguage()
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        ...
        <header className="mb-8">
          ...
          <h1 className="text-3xl font-bold mb-2">{rustModule.title}</h1>
          <p className="text-muted-foreground mb-4">
            Master Rust fundamentals for Solana development with 75+ hands-on exercises
          </p>
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <span>{rustModule.lessons.length} lessons</span>
            <span>•</span>
            <span>{rustModule.duration}</span>
          </div>
        </header>

        {/* Lessons List */}
        <div className="space-y-6">
          {rustModule.lessons.map((lesson) => (
                        ...
                        {lesson.exercises.map((exercise) => (
                          <Card key={exercise.id} className="p-4">
                            ...
                            <Button
                              size="sm"
                              onClick={() =>
                                router.push(`/learn/rust-programming-basics/practice/${lesson.id}/${exercise.id}`)
                              }
                            >
                              Start Exercise
